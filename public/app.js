...
function renderSpotifyPanel(status) {
  ...
  el.innerHTML = `
    <div class="spotify-connected-box">
      ...
      <div id="spotifyPlaylists" class="spotify-playlists"></div>
      ${status.isPremium ? `<div id="spotifyDevices" class="spotify-devices"></div>` : ''}
      <div id="spotifyPlayback" class="spotify-playback"></div>
...
async function loadSpotifyPlaylists() {
  try {
    ...
    el.innerHTML = `
      <h4 style="font-size:13px;color:#888;margin:16px 0 10px;text-transform:uppercase;letter-spacing:0.5px">
        Your Playlists
      </h4>
      ${playlists.map(p => `
        <div class="spotify-playlist-row">
          ${p.imageUrl
            ...
            : `<div style="width:40px;height:40px;border-radius:4px;background:#2a2a3a;flex-shrink:0"></div>`}
          ...
          <button onclick="loadPlaylistTracks('${p.id}', '${p.name.replace(/'/g, '').replace(/"/g, '')}')" class="btn-sm">
            View
          </button>
        </div>
      `).join('')}
    ...
    const el = document.getElementById('spotifyPlaylists');
    el.innerHTML = `
      <div style="background:#2a1010;border:1px solid #ff5c6a;border-radius:8px;padding:12px;margin-top:8px">
        <div style="color:#ff5c6a;font-size:12px;font-weight:600">Failed to load playlists</div>
        <div style="color:#aaa;font-size:11px;margin-top:4px">${e.message}</div>
      </div>
    `;
  }
}
